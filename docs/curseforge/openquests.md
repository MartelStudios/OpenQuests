...
*Combine quests as deep as you want and wire them together with AND and OR to get real branching progression. Beat the guardian with a sword, or find another way around.*

> 📦 **The chain above is a separate download.** OpenQuests installs with an empty quest list, on purpose: your server, your quests. Grab **OpenQuests Examples** to start with the line you see here, read it as a worked example, and delete it the day your own chain replaces it.
...
| Type | Completes on |
| --- | --- |
| `Gather` | Holding a quantity of an item |
| `InteractivelyPickup` | Picking a quantity up by hand |
...
| `EnterWorld` | Entering a world whose name matches a pattern |
| `Composite` | Its children, combined with `AND` or `OR` |
| `QuestState` | Another quest reaching a state, which is how you write a prerequisite |
| `Script` | Nothing on its own. Completed by a command or by your own plugin |
...
| `GrantQuest` | Hands the next quests over, which is how a chain continues |
| `Command` | Runs a server command, as the console or as the player |
...
| | |
| --- | --- |
| ⚔️ **OpenQuests** | this project: the quest types, the rewards and the tracker HUD |
| ⚙️ **OpenQuests Core** | the system underneath, pulled in with it and usable on its own |
...
Titles and descriptions are optional. A quest with no title names itself from its own parameters, so a `Gather` quest reads *Gather 2 Sticks* with nothing authored at all.
